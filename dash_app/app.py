...
# Initiate app
app = dash.Dash(name=__name__, external_stylesheets=[dbc.themes.ZEPHYR])
server = app.server
        ...
        dbc.Tabs(
            ...
            children=[
                dbc.Tab(
                    ...
                    tab_id="income_statement",
                    label_style={"color": "#448AFF"},
                ),
                dbc.Tab(
                    label="Balance Sheet",
                    tab_id="balance_sheet",
                    label_style={"color": "#448AFF"},
                ),
                dbc.Tab(
                    label="Cash Flow",
...
tabs_timerange = html.Div(
    [
        dbc.Tabs(
            ...
            active_tab="1y",
            children=[
                dbc.Tab(label="1D", tab_id="1d", label_style={"color": "#448AFF"}),
                dbc.Tab(label="5D", tab_id="5d", label_style={"color": "#448AFF"}),
                dbc.Tab(label="1M", tab_id="1mo", label_style={"color": "#448AFF"}),
                dbc.Tab(label="3M", tab_id="3mo", label_style={"color": "#448AFF"}),
                dbc.Tab(label="6M", tab_id="6mo", label_style={"color": "#448AFF"}),
                dbc.Tab(label="1Y", tab_id="1y", label_style={"color": "#448AFF"}),
                dbc.Tab(label="5Y", tab_id="5y", label_style={"color": "#448AFF"}),
                dbc.Tab(label="10Y", tab_id="10y", label_style={"color": "#448AFF"}),
                dbc.Tab(label="Max", tab_id="max", label_style={"color": "#448AFF"}),
            ],
        ),
    ]
)
    ...
    [
        dbc.Tabs(
            ...
            children=[
                ...
                dbc.Tab(
                    label="Yearly", tab_id="yearly", label_style={"color": "#448AFF"}
                ),
            ],
        ),
    ]
...
stock_price_graph = html.Div([dcc.Graph(id="stock_price_graph", figure=fig)])
...
report_graph_is = html.Div([dcc.Graph(id="report_graph_is", figure=report_fig_is)])
...
yoy_growth_fig_is = go.Figure(
    data=[
        ...
        go.Scatter(
            x=net_income_growth.index,
            y=net_income_growth.values,
            name="Revenue",
            mode="lines+markers",
        ),
    ]
...
yoy_growth_graph_is = html.Div(
    [dcc.Graph(id="yoy_growth_graph_is", figure=yoy_growth_fig_is)]
)
...
report_graph_bs = html.Div([dcc.Graph(id="report_graph_bs", figure=report_fig_bs)])
...
yoy_growth_fig_bs = go.Figure(
    data=[
        go.Scatter(
            x=asset_growth.index,
            y=asset_growth.values,
            name="Assets",
            mode="lines+markers",
        ),
        go.Scatter(
            x=liabilities_growth.index,
            y=liabilities_growth.values,
            name="Liabilities",
            mode="lines+markers",
        ),
        go.Scatter(
            x=equity_growth.index,
            y=equity_growth.values,
            name="Equity",
            mode="lines+markers",
        ),
    ]
...
yoy_growth_graph_bs = html.Div(
    [dcc.Graph(id="yoy_growth_graph_bs", figure=yoy_growth_fig_bs)]
)
...
report_fig_cf = go.Figure(
    data=[
        ...
        go.Bar(
            x=df_cf.columns,
            y=df_cf.loc["Cash from operating activities"],
            name="Cash Flow From Operating Activities",
        ),
        go.Bar(
            x=df_cf.columns,
            y=df_cf.loc["Cash from investing activities"],
            name="Cash Flow From Investing Activities",
        ),
        go.Bar(
            x=df_cf.columns,
            y=df_cf.loc["Cash from financing activities"],
...
report_graph_cf = html.Div([dcc.Graph(id="report_graph_cf", figure=report_fig_cf)])

# YoY growth graph
free_cash_flow_growth = df_cf.loc["Free cash flow"].pct_change() * 100
cash_flow_operating_growth = (
    df_cf.loc["Cash from operating activities"].pct_change() * 100
)
cash_flow_investing_growth = (
    df_cf.loc["Cash from investing activities"].pct_change() * 100
)
cash_flow_financing_growth = (
    df_cf.loc["Cash from financing activities"].pct_change() * 100
)

yoy_growth_fig_cf = go.Figure(
    data=[
        go.Scatter(
            x=free_cash_flow_growth.index,
            y=free_cash_flow_growth.values,
            name="Free Cash Flow",
            mode="lines+markers",
        ),
        go.Scatter(
            x=cash_flow_operating_growth.index,
            y=cash_flow_operating_growth.values,
            name="Cash Flow From Operating Activities",
            mode="lines+markers",
        ),
        go.Scatter(
            x=cash_flow_investing_growth.index,
            y=cash_flow_investing_growth.values,
            name="Cash Flow From Investing Activities",
            mode="lines+markers",
        ),
        go.Scatter(
            x=cash_flow_financing_growth.index,
            y=cash_flow_financing_growth.values,
            name="Cash Flow From Financing Activities",
            mode="lines+markers",
        ),
    ]
...
yoy_growth_graph_cf = html.Div(
    [dcc.Graph(id="yoy_growth_graph_cf", figure=yoy_growth_fig_cf)]
)
...
df_metrics = pd.DataFrame(
    ...
    index=[
        ...
        "Price to Free Cash Flow (PFCF) Ratio",
    ],
)
...
for ix, row in df_metrics.iterrows():
    if ix == "Net Income Growth":
        for year in [1, 3, 5]:
            df_metrics.loc[ix, f"{year} year"] = (
                str(
                    round(
                        (
                            (
                                df_is.loc["Net income"][-1]
                                - df_is.loc["Net income"][-1 - year]
                            )
                            / df_is.loc["Net income"][-1 - year]
                            * 100
                        ),
            ...
            df_metrics.loc[ix, f"{year} year"] = (
                str(
                    round(
                        (
                            (
                                df_bs.loc["Total equity"][-1]
                                - df_bs.loc["Total equity"][-1 - year]
                            )
                            / df_bs.loc["Total equity"][-1 - year]
                            * 100
                        ),
            ...
            df_metrics.loc[ix, f"{year} year"] = (
                str(
                    round(
                        (
                            (
                                df_cf.loc["Free cash flow"][-1]
                                - df_cf.loc["Free cash flow"][-1 - year]
                            )
                            / df_cf.loc["Free cash flow"][-1 - year]
                            * 100
                        ),
        ...
        for year in [1, 3, 5]:
            if not all(pd.isna(df_ra.loc["Return on assets %"][-1 - year : -1])):
                df_metrics.loc[ix, f"{year} year"] = (
                    str(
                        round(
                            np.nanmean(df_ra.loc["Return on assets %"][-1 - year : -1]),
                            2,
                        )
                    )
                    + " %"
        ...
        for year in [1, 3, 5]:
            if not all(pd.isna(df_ra.loc["Return on equity %"][-1 - year : -1])):
                df_metrics.loc[ix, f"{year} year"] = (
                    str(
                        round(
                            np.nanmean(df_ra.loc["Return on equity %"][-1 - year : -1]),
                            2,
                        )
                    )
                    + " %"
        ...
        for year in [1, 3, 5]:
            if not all(pd.isna(df_ra.loc["Debt to equity ratio"][-1 - year : -1])):
                df_metrics.loc[ix, f"{year} year"] = str(
                    round(
                        np.nanmean(df_ra.loc["Debt to equity ratio"][-1 - year : -1]), 2
                    )
                )
        ...
        for year in [1, 3, 5]:
            if not all(pd.isna(df_ra.loc["Current ratio"][-1 - year : -1])):
                df_metrics.loc[ix, f"{year} year"] = str(
                    round(np.nanmean(df_ra.loc["Current ratio"][-1 - year : -1]), 2)
                )
                if float(df_metrics.loc[ix, f"{year} year"].replace(" %", "")) > 1:
                    df_metrics.loc[ix, "Indicator"] = "✅"
                else:
        ...
        for year in [1, 3, 5]:
            if not all(pd.isna(df_ra.loc["Quick ratio"][-1 - year : -1])):
                df_metrics.loc[ix, f"{year} year"] = str(
                    round(np.nanmean(df_ra.loc["Quick ratio"][-1 - year : -1]), 2)
                )
                if float(df_metrics.loc[ix, f"{year} year"].replace(" %", "")) > 1:
                    df_metrics.loc[ix, "Indicator"] = "✅"
                else:
        ...
        for year in [1, 3, 5]:
            if not all(pd.isna(df_ra.loc["Price to earnings ratio"][-1 - year : -1])):
                df_metrics.loc[ix, f"{year} year"] = str(
                    round(
                        np.nanmean(
                            df_ra.loc["Price to earnings ratio"][-1 - year : -1]
                        ),
                        2,
                    )
                )
        ...
        for year in [1, 3, 5]:
            if not all(pd.isna(df_ra.loc["Price to book ratio"][-1 - year : -1])):
                df_metrics.loc[ix, f"{year} year"] = str(
                    round(np.mean(df_ra.loc["Price to book ratio"][-1 - year : -1]), 2)
                )
                if float(df_metrics.loc[ix, f"{year} year"].replace(" %", "")) < 1:
                    df_metrics.loc[ix, "Indicator"] = "✅"
                else:
        ...
        for year in [1, 3, 5]:
            if not all(pd.isna(df_ra.loc["Price to cash flow ratio"][-1 - year : -1])):
                df_metrics.loc[ix, f"{year} year"] = str(
                    round(
                        np.mean(df_ra.loc["Price to cash flow ratio"][-1 - year : -1]),
                        2,
                    )
                )
                if float(df_metrics.loc[ix, f"{year} year"].replace(" %", "")) < 20:
...
for index in df_datatable.drop(["Variable", "Unit"], axis=1).index:
    if all(
        abs(
            df_datatable.drop(["Variable", "Unit"], axis=1)
            .loc[index]
            .apply(lambda x: float(x) / 1000 if x not in [0, "-"] else 1)
        )
        >= 1
    ):
        df_datatable.loc[
            index,
            np.logical_and(
                df_datatable.columns != "Variable", df_datatable.columns != "Unit"
            ),
        ] = (
            df_datatable.loc[
                index,
            ...
            .apply(lambda x: float(x) / 1000 if x not in ["-"] else "-")
            .apply(lambda x: str(x) if x not in ["-", "nan", 0] else "-")
        )
    ...
    if all(
        abs(
            ...
            .loc[index]
            .apply(
                lambda x: float(str(x).replace(" K", "")) / 1000
                if x not in [0, "-"]
                else 1
            )
        )
        ...
        df_datatable.loc[
            ...
            .apply(
                lambda x: float(x.replace(" K", "")) * 1000 / 1000000
                if x not in ["-"]
                else "-"
            )
            .apply(lambda x: str(x) if x not in ["-", "nan", 0] else "-")
    ...
    if all(
        abs(
            ...
            .loc[index]
            .apply(
                lambda x: float(str(x).replace(" M", "")) / 1000
                if x not in [0, "-"]
                else 1
            )
        )
        ...
        df_datatable.loc[
            ...
            .apply(
                lambda x: float(x.replace(" M", "")) * 1000000 / 1000000000
                if x not in ["-"]
                else "-"
            )
            .apply(lambda x: str(x) if x not in ["-", "nan", 0] else "-")
    ...
    if all(
        abs(
            ...
            .loc[index]
            .apply(
                lambda x: float(str(x).replace(" B", "")) / 1000
                if x not in [0, "-"]
                else 1
            )
        )
        ...
        df_datatable.loc[
            ...
            .apply(
                lambda x: float(x.replace(" B", "")) * 1000000000 / 1000000000000
                if x not in ["-"]
                else "-"
            )
            .apply(lambda x: str(x) if x not in ["-", "nan", 0] else "-")
...
app.layout = html.Div(
    [
        ...
        html.P(
            children="A dashboard to analyze stocks from IDX (Indonesian Stock Exchange)",
            style={
                "fontSize": 18,
                "textAlign": "center",
                # "margin-top": "0px",
        ...
        dbc.Container(
            children=[
                dbc.Row(
                    [
                        dbc.Col([dropdown_ticker], xs=12, sm=12, md=12, lg=3, xl=3),
                        dbc.Col(
                            [
                                dbc.Row([tabs_timerange]),
                                dbc.Row([stock_price_graph]),
                            ],
                            xs=12,
                            sm=12,
                            md=12,
                ...
                # ], align="center", justify="center"),
                dbc.Row(
                    [dbc.Col([metrics_table], xs=10, sm=10, md=10, lg=8, xl=8)],
                    align="center",
                    justify="center",
                ),
                dbc.Row(
                    [dbc.Col([tabs_statement], xs=12, sm=12, md=12, lg=10, xl=10)],
                    align="center",
                    justify="center",
                ),
                dbc.Row(
                    [
                        dbc.Col([report_graph_is], xs=12, sm=12, md=12, lg=5, xl=5),
                        dbc.Col([yoy_growth_graph_is], xs=12, sm=12, md=12, lg=5, xl=5),
                    ],
                    align="center",
                    justify="center",
                ),
                dbc.Row(
                    [
                        dbc.Col([report_graph_bs], xs=12, sm=12, md=12, lg=5, xl=5),
                        dbc.Col([yoy_growth_graph_bs], xs=12, sm=12, md=12, lg=5, xl=5),
                    ],
                    align="center",
                    justify="center",
                ),
                dbc.Row(
                    [
                        dbc.Col([report_graph_cf], xs=12, sm=12, md=12, lg=5, xl=5),
                        dbc.Col([yoy_growth_graph_cf], xs=12, sm=12, md=12, lg=5, xl=5),
                    ],
                    align="center",
                    justify="center",
                ),
                dbc.Row(
                    [dbc.Col([tabs_report], xs=12, sm=12, md=12, lg=10, xl=10)],
                    align="center",
                    justify="center",
                ),
                dbc.Row(
                    [dbc.Col([data_table], xs=12, sm=12, md=12, lg=10, xl=10)],
                    align="center",
                    justify="center",
                ),
            ],
...
def update_data(
    value_dropdown_ticker, value_tabs_report, value_tabs_statement, value_tabs_timerange
):
    # Update stockprice
    ...
    stock_price = (
        yf.Ticker(f"{value_dropdown_ticker}.JK")
    ...
    report_fig_bs = go.Figure(
        data=[
            go.Bar(x=df_bs.columns, y=df_bs.loc["Total assets"], name="Assets"),
            go.Bar(
                x=df_bs.columns, y=df_bs.loc["Total liabilities"], name="Liabilities"
            ),
            go.Bar(x=df_bs.columns, y=df_bs.loc["Total equity"], name="Equity"),
        ]
    )
    ...
    report_fig_cf = go.Figure(
        data=[
            go.Bar(
                x=df_cf.columns, y=df_cf.loc["Free cash flow"], name="Free Cash Flow"
            ),
            go.Bar(
                x=df_cf.columns,
                y=df_cf.loc["Cash from operating activities"],
        ...
        if all(
            ...
            df_datatable.loc[
                ...
                df_datatable.loc[
                    index,
                    np.logical_and(
                        df_datatable.columns != "Variable",
                        df_datatable.columns != "Unit",
                    ),
                ]
                .apply(lambda x: float(x) / 1000 if x not in ["-"] else "-")
                .apply(lambda x: str(x) if x not in ["-", "nan", 0] else "-")
    ...
    df_metrics = pd.DataFrame(
        columns=[
            f"1 {value_tabs_statement}",
            f"3 {value_tabs_statement}",
            f"5 {value_tabs_statement}",
            "Indicator",
            "Parameter",
        ],
        index=[
    ...
    for ix, row in df_metrics.iterrows():
        if ix == "Net Income Growth":
            for year in [1, 3, 5]:
                df_metrics.loc[ix, f"{year} {value_tabs_statement}"] = (
                    str(
                        round(
                            (
                                abs(
                ...
                if not all(pd.isna(df_ra.loc["Return on assets %"][-1 - year : -1])):
                    df_metrics.loc[ix, f"{year} {value_tabs_statement}"] = (
                        str(
                            round(
                                np.nanmean(
                                    df_ra.loc["Return on assets %"][-1 - year : -1]
                                ),
                                2,
                            )
                        )
                ...
                if not all(pd.isna(df_ra.loc["Return on equity %"][-1 - year : -1])):
                    df_metrics.loc[ix, f"{year} {value_tabs_statement}"] = (
                        str(
                            round(
                                np.nanmean(
                                    df_ra.loc["Return on equity %"][-1 - year : -1]
                                ),
                                2,
                            )
                        )
            ...
            for year in [1, 3, 5]:
                if not all(pd.isna(df_ra.loc["Debt to equity ratio"][-1 - year : -1])):
                    df_metrics.loc[ix, f"{year} {value_tabs_statement}"] = str(
                        round(
                            np.nanmean(
                                df_ra.loc["Debt to equity ratio"][-1 - year : -1]
                            ),
                            2,
                        )
                    )
            ...
            for year in [1, 3, 5]:
                if not all(
                    pd.isna(df_ra.loc["Price to earnings ratio"][-1 - year : -1])
                ):
                    df_metrics.loc[ix, f"{year} {value_tabs_statement}"] = str(
                        round(
                            np.nanmean(
            ...
            for year in [1, 3, 5]:
                if not all(pd.isna(df_ra.loc["Price to book ratio"][-1 - year : -1])):
                    df_metrics.loc[ix, f"{year} {value_tabs_statement}"] = str(
                        round(
                            np.mean(df_ra.loc["Price to book ratio"][-1 - year : -1]), 2
                        )
                    )
            if all(
                df_metrics.loc[ix][
            ...
            for year in [1, 3, 5]:
                if not all(
                    pd.isna(df_ra.loc["Price to cash flow ratio"][-1 - year : -1])
                ):
                    df_metrics.loc[ix, f"{year} {value_tabs_statement}"] = str(
                        round(
                            np.mean(
                                df_ra.loc["Price to cash flow ratio"][-1 - year : -1]
                            ),
                            2,
                        )
                    )
