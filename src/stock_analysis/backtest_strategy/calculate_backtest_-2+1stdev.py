    ...
    for multiple in ["pe", "pbv"]:
        ...
        for tf in timeframes:
            ...
            for ix, row in df.iterrows():
                if row[f"{multiple}"] < row[f"mean_-2_stdev_{tf}y"] and bought == False:
                    # col = determine_quarter_col(ix)
                    # year = ix.year
...
with open(f"{dir}/idx_30_backtest_{date_str}_-2+1stdev.json", "w") as outfile:
