...
# # Get release date
# collection_release_date = db_stockbit_data["release_dates"]
# release_date = collection_release_date.find_one({"stock_code": stock})
# df_release_date = (
#     pd.DataFrame(release_date)
#     .reset_index()
#     .rename(columns={"index": "Period", "release_dates": "Date"})
# )
# df_release_date["Date"] = df_release_date["Date"].apply(
#     lambda x: x.replace(second=0, minute=0, hour=0)
# )
...
for ix, values in stock_price.iterrows():
    ...
    try:
        ...
        pe.loc[ix, "PE"] = values.Close / (
            ...
            / (float(df_is.loc["Share Outstanding"][0]) * 1000)
        )
...
# # Add release date
# net_income_ttm = pd.merge(
#     net_income_ttm, df_release_date[["Period", "Date"]], on="Period", how="left"
# )
# book_value_quarterly = pd.merge(
#     book_value_quarterly,
#     df_release_date[["Period", "Date"]],
#     on="Period",
#     how="left",
# )
...
net_income_chart = (
    ...
    .mark_bar(cornerRadiusTopLeft=5, cornerRadiusTopRight=5, color="#FF9800", size=3)
    .encode(
        alt.X(
            "Date:T",
...
chart = (
    alt.Chart(stock_price.reset_index())
    .mark_area(
        line={"color": "#448AFF"},
        color=alt.Gradient(
            gradient="linear",
...
price_chart = alt.layer(chart)
# price_net_income_chart = alt.layer(chart, net_income_chart)
# price_book_value_chart = alt.layer(chart, book_value_chart)
...
# st.altair_chart(
#     price_net_income_chart,
#     use_container_width=True,
# )

# st.altair_chart(
#     price_book_value_chart,
#     use_container_width=True,
# )
