...
class StockbitScraper:
    ...
    def login(self, username, password):
        ...
        sleep(3)
        input_username = self.driver.find_element_by_xpath('//input[@id="username"]')
        input_username.send_keys(username)
        input_password = self.driver.find_element_by_xpath('//input[@id="password"]')
        input_password.send_keys(password)

        # Click login button
        self.driver.find_element_by_xpath('//input[@id="loginbutton"]').click()
        sleep(3)

    def get_fundamental_data(
        self,
        stock_filter: Optional[list] = ALL,
        ...
        # Create directory
        directory = f'./results/stockbit'
        create_directory(f"{directory}/yearly")
        create_directory(f"{directory}/quarterly")
        ...
        with alive_bar(len(stocks), force_tty=True) as bar:
            for stock in stocks:
                try:
                    ...
                    json_structure_yearly = {}
                    json_structure_quarterly = {}

                    selection_report_type = Select(self.driver.find_element(By.CLASS_NAME, 'reportType'))
                    selection_statement_type= Select(self.driver.find_element(By.CLASS_NAME, 'statement-type'))
                    
                    # Select always EN
                    Select(self.driver.find_element(By.CLASS_NAME, 'transType')).select_by_value('1')
                    sleep(5)

                    for report_type in report_types:
                        if report_type == 'income-statement':
                            selection_report_type.select_by_value('is')
                        elif report_type == 'balance-sheet':
                            selection_report_type.select_by_value('bs')
                        elif report_type == 'cash-flow':
                            selection_report_type.select_by_value('cf')
                        sleep(5)

                        for statement_type in statement_types:
                            if statement_type == 'quarterly':
                                selection_statement_type.select_by_value('QUARTERLY')
                            elif statement_type == 'yearly':
                                selection_statement_type.select_by_value('ANNUALLY')
                            sleep(5)

                            # Always round to K
                            self.driver.find_element(By.XPATH, '//*[@id="content-box"]/div[3]/div[2]/div[1]/div[8]/div[3]/button[2]').click()
                            self.driver.find_element(By.XPATH, '//*[@id="content-box"]/div[3]/div[2]/div[1]/div[8]/div[3]/button[2]').click()
                            self.driver.find_element(By.XPATH, '//*[@id="content-box"]/div[3]/div[2]/div[1]/div[8]/div[3]/button[2]').click()
                        
                            tables = pd.read_html(self.driver.page_source)
                            df1 = tables[1].rename(columns={"In Thousand IDR": "In IDR"})
                            df2 = tables[2]
                            ...
                            for col in data.columns:
                                data[col] = data[col].apply(lambda x: str(x).replace(',','').replace(' K','000').replace(' B','000000').replace('-','').replace(')','').replace('(','-'))
                            ...
                            if report_type == 'income-statement':
                                ...
                                if statement_type == 'quarterly':
                                    json_structure_quarterly.update({"income_statement": json.loads(data.to_json(orient='split', indent=4))})
                                if statement_type == 'yearly':
                                    json_structure_yearly.update({"income_statement": json.loads(data.to_json(orient='split', indent=4))})
                            ...
                            if report_type == 'balance-sheet':
                                ...
                                if statement_type == 'quarterly':
                                    json_structure_quarterly.update({"balance_sheet": json.loads(data.to_json(orient='split', indent=4))})
                                if statement_type == 'yearly':
                                    json_structure_yearly.update({"balance_sheet": json.loads(data.to_json(orient='split', indent=4))})
                            ...
                            if report_type == 'cash-flow':
                                ...
                                if statement_type == 'quarterly':
                                    json_structure_quarterly.update({"cash_flow": json.loads(data.to_json(orient='split', indent=4))})
                                if statement_type == 'yearly':
                                    json_structure_yearly.update({"cash_flow": json.loads(data.to_json(orient='split', indent=4))})
                    
                    # Save the data in json
                    with open(f'{directory}/yearly/{stock}.json', 'w') as f:
                        f.write(json.dumps(json_structure_yearly, ensure_ascii=False, indent=4))
                    with open(f'{directory}/quarterly/{stock}.json', 'w') as f:
                        f.write(json.dumps(json_structure_quarterly, ensure_ascii=False, indent=4))
