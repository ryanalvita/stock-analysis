...
class NotifikasiEmailRilisLapkeu:
    ...
    def get_all_releases(
        ...
        while True:
            ...
            elements = self.driver.find_elements(
                By.XPATH, "/html/body/main/div[2]/div/div[2]/div/div"
            )
            i = 1
                ...
                if time < datetime_yesterday:
                    ...
                    try:
                        text_lapkeu = [
                            text
                            for text in texts
                            if "Penyampaian Laporan Keuangan" in text
                        ][0]

                        ticker = text_lapkeu[-6:-2]
                        text_financial = [
                            text for text in texts if "FinancialStatement" in text
                        ][0].split("-")
                        ...
                        link = self.driver.find_element(
                            By.XPATH,
                            f"/html/body/main/div[2]/div/div[2]/div/div[{i}]/a",
                        ).get_attribute("href")
            ...
            if time < datetime_yesterday:
                ...
                period_click = self.driver.find_element(
                    By.XPATH,
                    "/html/body/main/div[2]/div/dir-pagination-controls/ul/li[11]/a",
                )
                self.driver.execute_script("arguments[0].click();", period_click)
    ...
    def send_email(
        ...
        html = html.replace(
            "1 Oktober 2020", datetime.strftime(datetime_yesterday, "%d %B %Y")
        )
